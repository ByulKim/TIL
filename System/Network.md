...
- 데이터를 전송할 때 출발지 ip, 도착지 ip, 데이터 등을 담은 패킷의 형태로 전송한다.
- IP의 한계
    - 비연결성
...
- 출발지 port, 도착지 port, 전송 제어, 순서, 검증 정보를 담아 전송한다.
    - port
        - 프로세스 구분
        - 같은 ip 내에서 전송할 프로세스(프로그램)을 지정한다.

- 특성
    - 연결 지향 : 미리 연결을 하고 데이터를 전송한다.
        - 3 way handshake
            - syn (synchronize) : 클라이언트에서 요청, 연결해달라는 의미
            - syn + ack : 서버의 응답, 서버 또한 연결해달라는 의미
            - akc : 클라이언트의 응답
            - 연결 이후에 데이터 전송 (마지막 ack 와 함께 데이터 전송도 가능함)
            - 논리적인 연결이 되었다고 판단하는 것이다.
                - 실제로 어떤 랜선이나 경로가 확실히 보장되는 것은 아니다.

    - 데이터 전달 보장
        - 데이터 전송 후에 잘 받았는지에 대한 응답을 받아 확인한다.
    - 순서 보장
        - 잘못 받은 패킷 앞에부터 다시 보내라고 응답한다.
...
- 기능이 거의 없음 (TCP의 기능인 데이터 연결, 전달, 순서를 보장하지 않는다.)
- 출발지 port, 도착지 port, 체크섬(메시지 검증) 정도만 포함된다.
- 단순하고 빠르다.
- 직접 최적화 할 수 있다.
