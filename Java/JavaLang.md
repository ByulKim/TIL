...
- 기본형 타입의 한계
    - 객체가 아니어서 유용한 메서드들을 제공할 수 없다.
        - 자신의 값과 다른 값을 비교하는 메서드가 있다면 좋을 것이다.
    - null 값을 가질 수 없다.
        - 멤버변수로 사용하면 기본값이, 지역변수로 사용하면 초기화 하고 사용해야 한다.
        - 없거나 찾지 못했거나 정해지지 않았거나 하는 상태를 표현할 수 없다.
- 기본형의 객체 버전 = 기본형을 객체로 감싼 것이다.
- 자바는 기본형에 대응하는 래퍼 클래스를 기본으로 제공한다.
    - `Integer`,`Long`,`Double`,`Boolean`등
- 불변 클래스이다.
- `equals()`메서드로 비교해야 한다.

```java
Integer value = new Integer(10); // 곧 없어질 예정이라 사용 권장하지 않음
Integer value = Integer.valueOf(10); // 메서드를 통해 객체 생성하는 것 권장됨
```

- `valueOf()`메서드를 사용하면 해당 메서드 내부에서`Integer`객체를 생성 후 반환한다.
- 문자열 풀과 비슷하게 자주 사용하는 숫자를 미리 만들어두고 재사용한다. (최적화 기능)
    - 미리 생성된`Integer`갹체의 참조값을 반환한다.
- `intValue()`: 언박싱 메서드
    - 래퍼 클래스에 들어있는 기본형 값을 다시 꺼내는 메서드
- 래퍼 클래스는 기본형처럼 값을 직접 출력해도 주소값이 아닌 값이 출력되도록 설정되어 있다.


- 기능
    - `valueOf()`: 기본형 변수/문자열을 래퍼 객체로 변환
    - `parseInt()`: 문자열을 기본형 변수로 변환
    - static 메서드
        - `sum(a, b)`: 둘의 합을 반환한다.
        - `min(a, b), max(a, b)`: 둘 중 작은수/큰수 를 반환한다.


- 래퍼클래스가 기능까지 있어 더 좋은데, 왜 기본형 타입을 사용하나?
    - 성능
        - 기본형의 연산이 보통 래퍼클래스보다 빠르다.
        - 기본형은 메모리에서 그 크기만큼의 공간을 차지하지만, 래퍼 클래스는 인스턴스 이기에 필드로 가지는 기본형의 값 뿐만 아니라객체를 다루는데 필요한 객체 메타 데이터를 포함하여 더 많은 메모리를
          사용한다.
        - → 시간, 메모리 (성능) 측면에서 기본형이 더 좋은 점이 있다.
    - 하지만, 정말 많은 수의 연산을 수행하지 않는 이상 큰 차이는 없다.
    - 해서 코드를 유지보수 하기에 더 나은 방향으로 선택해서 사용하면 된다.

> 유지보수와 최적화 고민
> - 유지보수 하기 좋은 코드를 먼저 고민하는 것이 좋다. (컴퓨터의 성능이 기본적으로 많이 좋아졌기 때문)
> - 성능 최적화를 위해 유지보수 해야하는 코드가 늘어날 수 있다.
> - 성능 관점에서 불필요한 최적화를 할 가능성이 있다.
...
Integer boxedValue = Integer.valueOf(5); // 박싱 (boxing)
Integer boxedValue = 5; // 오토 박싱 (auto-boxing)

int unboxedValue = boxedValue.intValue(); // 언박싱 (unboxing)
int unboxedValue = boxedValue; // 오토 언박싱 (auto-unboxing)
```

- 자동으로 기본형에서 래퍼 클래스로, 래퍼 클래스에서 기본형으로 변환한다.
...
- 기능 (static 메서드)
    ...
    - `currentTimeMillis()`: 현재 시간을 밀리초로 가져온다.
    - `nanoTime()`: 현재 시간을 나노초로 가져온다.
    - `getenv()`: (운영체체와 관련된) 환경 변수를 읽는다.
    - `getProperties()`: (자바와 관련된) 시스템 속성을 읽는다.
    - `arraycopy()`: 배열을 고속으로 복사한다.
        - 시스템 레벨에서 통으로 복사하기 때문에 직접 반복문을 통해 값을 복사하는 것보다 속도가 빠르다.
    - `exit()`: 프로그램을 종료한다.
        - 프로그램을 강제로 종료하기 때문에 가급적 사용을 권장하지 않는다.
    - GC 수행
